...
class Http {
  ...
  constructor(){
    ...
    this.api = {};
    this.apiConfig = {"domainUrl":"","header":{'Content-Type':'application/json;charset=utf-8'}};
  }
  ...
  setContentType(_string=''){
    ...
    this.apiConfig['header']['Content-Type'] = _string;
  }
  ...
  setDomainUrl(_preUrl){
    this.apiConfig['domainUrl'] = _preUrl;
  }
  setApi(_id,_url,_domain,_format,mock){
    ...
    this.api[_id] = {"url":(_domainUrl||this.apiConfig['domainUrl'])+_url,"mock":mock||"","domain":_domain||this.apiConfig['domainUrl'],"format":isFormat};
    return this.api;
  }
  getApi(_id,_target='url'){
    if(typeof _id === 'undefined'){
      ...
      throw Error(_id+"--->"+"无法找到该别名下的"+_target+"属性，请确认是否配置！");
    }
  }
  ...
  getMock(_id){
    if(this.api.hasOwnProperty(_id)&&this.api[_id].hasOwnProperty('mock')){
      ...
      throw Error(_id+"--->"+"找不到该别名下的接口及模拟数据，请确认是否配置！");
    }
  }
  ...
  setMutiApi(_arr){
    if(_arr instanceof Array){
      for(let i=0,len=_arr.length;i<len;i++){
       ...
       throw new Error(JSON.stringify(_arr)+'--->'+'参数必须为数组！期望是::[{},{}...]');
    }
  }
  ...
  handle(_id,_method,_data,_callback){
    ...
    if(requestConfig['headers']['Content-Type'].toLowerCase() === 'application/x-www-form-urlencoded;charset=utf-8'){
      _data = that.transGetParams(_data);
    }
      ...
      .catch(function (error) {
        //todo 异常统一处理，暂时只抛出
          ...
          if (typeof error.response === 'undefined') {
              that.error('网络异常：您可能断网了！');
          } else {
              let err = ""
              switch(error.response.status){
                  case 400:
                      err = " 1、语义有误，当前请求无法被服务器理解。除非进行修改，否则客户端不应该重复提交这个请求。 　　2、请求参数有误。"
                      break
                  case 404:
                      err = "请求失败，请求所希望得到的资源未被在服务器上发现。"
                      break
                  case 405:
                      err = "请求行中指定的请求方法不能被用于请求相应的资源。"
                      break
                  case 500:
                      err = "服务器遇到了一个未曾预料的状况，导致了它无法完成对请求的处理。一般来说，这个问题都会在服务器的程序码出错时出现。"
                      break
                  case 501:
                      err = " 服务器不支持当前请求所需要的某个功能。当服务器无法识别请求的方法，并且无法支持其对任何资源的请求。"
                      break
                  case 502:
                      err = " 作为网关或者代理工作的服务器尝试执行请求时，从上游服务器接收到无效的响应。"
                      break
                  case 503:
                      err = " 由于临时的服务器维护或者过载，服务器当前无法处理请求。"
                      break
                  case 504:
                      err = "作为网关或者代理工作的服务器尝试执行请求时，未能及时从上游服务器（URI标识出的服务器，例如HTTP、FTP、LDAP）或者辅助服务器（例如DNS）收到响应。 "
                      break
                  case 505:
                      err = " 服务器不支持，或者拒绝支持在请求中使用的 HTTP 版本。这暗示着服务器不能或不愿使用与客户端相同的版本。响应中应当包含一个描述了为何版本不被支持以及服务器支持哪些协议的实体。"
                      break
                  default:
                      err = '前端捕获请求前异常：：'+error
              }
